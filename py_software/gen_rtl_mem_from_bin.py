...
The converter walks nested subdirectories so stage-specific exports can live
under their own folders without changing the conversion flow.
...
def main():
    ...
    bin_files = sorted(p for p in gv_dir.rglob("*.bin") if "rtl_mem" not in p.parts)
    if not bin_files:
    ...
    for bin_path in bin_files:
        try:
            rel = bin_path.relative_to(gv_dir)
            dst_dir = out_dir / rel.parent
            dst_dir.mkdir(parents=True, exist_ok=True)
            out_path = dst_dir / (bin_path.stem + "_q312.mem")
            ...
            print(f"[INFO] {bin_path.relative_to(gv_dir)}: {arr.shape} -> {q.size} samples -> {out_path.relative_to(gv_dir)}")
