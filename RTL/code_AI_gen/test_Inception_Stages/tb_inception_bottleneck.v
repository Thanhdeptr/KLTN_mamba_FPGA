...
module tb_inception_bottleneck;
    ...
    integer fd_out, fd_log, token_count, out_count, ch, offset, timeout;
    reg [15:0] input_mem [0:63999];

    Inception_Bottleneck_Stage dut (
        .clk(clk),
        .rst_n(rst_n),
    ...
    initial begin
        ...
        if ((fd_out == 0) || (fd_log == 0)) begin
            $display("[ERROR] Cannot open output files");
            $finish;
        end

        $readmemh("../test_BaseInceptionBlock_Full/weights_and_io/inception_bottleneck_weight_q312.mem", dut.weights_bn);
