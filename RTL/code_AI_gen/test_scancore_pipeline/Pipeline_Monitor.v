...
module Pipeline_Monitor #(
    ...
    // monitor directory buffer and flag
    reg [8*256-1:0] mon_dir;
    integer got_mon_dir;

    initial begin
        // optional override for output directory via +MONITOR_DIR=<path>
        got_mon_dir = 0;
        got_mon_dir = $value$plusargs("MONITOR_DIR=%s", mon_dir);
        cycle_cnt = 0;
        prev_rms_start = 0;
        prev_rms_done = 0;
        prev_inproj_start = 0;
        ...
        if (got_mon_dir) begin
            fd_lat   = $fopen({mon_dir, "/latency_report.csv"}, "w");
            fd_state = $fopen({mon_dir, "/state_cycles.csv"}, "w");
        end else begin
        ...
        if (reset) begin
            ...
            if (monitor_en) begin
                ...
                if (outproj_done && !prev_outproj_done) begin
                    ...
                    $display("PIPELINE_MONITOR: dumped state_counters for tx=%0d at cycle=%0d (outproj_done)", tx_id, cycle_cnt);
                end
                // Fallback: if outproj_done is not used in this testbench, also dump when scan completes
                else if (all_scan_done && !prev_all_scan_done) begin
                    for (s = 0; s < NUM_STATES; s = s + 1) begin
                        if (fd_state) $fdisplay(fd_state, "%0d,%0d,%0d", tx_id, s, state_counters[s]);
                        state_counters[s] <= 0;
                    end
                    $display("PIPELINE_MONITOR: dumped state_counters for tx=%0d at cycle=%0d (all_scan_done fallback)", tx_id, cycle_cnt);
                end
            end
